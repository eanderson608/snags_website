...
var formatGoogleCalendar = (function() {
    ...
    var transformationList = function(result, tagName, format) {
        ...
        for (i = 0; i < format.length; i++) {
            ...
            if (format[i] === '*summary*') {
                output = output.concat('<h3 class="summary">' + summary + '</h3>');
    ...
    //Add one day (fixes issue where all day events display as previous day, fix gotten from https://github.com/MilanKacurak/FormatGoogleCalendar/issues/12)
    ...
    var formatDateSameDay = function(dateStart, dateEnd, moreDaysEvent, isAllDayEvent, dayNames) {
        ...
        if (config.sameDayTimes && !moreDaysEvent && !isAllDayEvent) {
            ...
            // if start time is equal to end time dont display end time
            if (dateStart[3] === dateEnd[3]) {
                formattedTime = ' at ' + getFormattedTime(dateStart)
            } else {
